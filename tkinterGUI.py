...
tree_columns = ['Item','Rating','Price','kg','Emissions']
choice_tree_columns = ['Emissions per kg','Emissions per portion','Emissions per calorie']
choice = 0
...
def search_products(search_term, sort_by, desc=True):
    ...
    ORDER BY ***sort_by*** ***direction***'''.replace('***condition***', ' AND '.join(["(LOWER(sainsburys_products.name) LIKE '%x%' OR LOWER(sainsburys_sectors.path) LIKE '%x%')".replace('x',str.lower(x)) for x in search_term.split(' ')])).replace('***sort_by***',sort_by).replace('***direction***','DESC' if desc else 'ASC')
    print(sql_text)
...
def search_page(search_term, sort_by, desc=False):
    ...
    tree = ttk.Treeview(root, columns=data_columns+[choice_data_columns[choice]], show='headings', height=(int(root.grid_bbox(1,1)[3]*10-25))//30)
